...
def update_profile(request):
    ...
    if request.method == 'POST':
        ...
        if user_email and user_email != user.email:
            ...
            try:
                send_mail(mail_subject, '', from_email, [user_email], html_message=message)
                messages.success(request, _("Email submitted successfully! Please check your email to verify your Email."))
            except Exception as e:
                logger.warning(f"SMTP failed to send email verification for {user_email}: {e}")
                messages.warning(request, _("Unable to send verification email via SMTP. Please contact administrator."))
